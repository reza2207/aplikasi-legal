<div class="row first" style="min-height: 510px">
  ...
  <div class="col push-l3 l4 pull-5 s12">
    ...
    <select id="sortdata">
      <option value="vendor_id" <?= $sort_by == 'vendor_id' ? 'selected' :'';?>>Vendor Id</option>
      <option value="nama_vendor" <?= $sort_by == 'nama_vendor' ? 'selected' :'';?>>Nama Vendor</option>
      <option value="alamat" <?= $sort_by == 'alamat' ? 'selected' :'';?>>Alamat</option>
      <option value="kota" <?= $sort_by == 'kota' ? 'selected' :'';?>>Kota</option>
      <option value="kode_pos" <?= $sort_by == 'kode_pos' ? 'selected' :'';?>>Kode Pos</option>
      <option value="telp_1" <?= $sort_by == 'telp_1' ? 'selected' :'';?>>Telp 1</option>
      <option value="telp_2" <?= $sort_by == 'telp_2' ? 'selected' :'';?>>Telp 2</option>
      <option value="fax" <?= $sort_by == 'fax' ? 'selected' :'';?>>No. Fax</option>
      <option value="email" <?= $sort_by == 'email' ? 'selected' :'';?>>Email</option>
      <option value="bidang_usaha" <?= $sort_by == 'bidang_usaha' ? 'selected' :'';?>>Bidang Usaha</option>
      <option value="npwp" <?= $sort_by == 'npwp' ? 'selected' :'';?>>NPWP</option>
      <option value="keterangan" <?= $sort_by == 'keterangan' ? 'selected' :'';?>>Keterangan</option>
      <option value="unit" <?= $sort_by == 'unit' ? 'selected' :'';?>>Unit</option>
    </select>
    <!-- <span id="result">Searching result  record (s) ....</span> -->
  </div>
